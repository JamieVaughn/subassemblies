...
// Confetto Class
function Confetto() {
  let randomModifier = randomRange(0, 99)
  ...
  let rotation = randomRange(0, 2 * Math.PI)
  let scale = {
    x: 1,
    y: 1,
  }
  let velocity = initConfettoVelocity([-9, 9], [6, 11])
  const update = () => {
  ...
  return {
    color, dimensions, position, rotation, scale, velocity, update
  }
}
...
// Sequin Class
const Sequin = () => {
  let color = colors[Math.floor(randomRange(0, colors.length))].back
  let radius = randomRange(1, 2)
  let position = {
  ...
  return {
    color, radius, position, velocity, update
  }
}
...
<button bind:this={button} id="button" class="ready" on:click={clickButton}>
